...
const components = {
      ...
      <section class="search-section">
        <div class="search-container">
          ...
          <input 
            ...
            class="search-input" 
            id="searchInput"
            placeholder="Buscar tipo de mutación, mecanismo, enfermedad, ejemplo..."
  ...
  createCard(mutacion, index) {
    const isExpanded = state.expandedCards.has(mutacion.id);
    ...
    return `
      ...
      >
        <div class="card-header" data-id="${mutacion.id}">
          <div class="card-icon">${mutacion.icono}</div>
          <div class="card-title-area">
            <h2 class="card-title">${shouldHighlight ? utils.highlightText(mutacion.nombre, state.searchTerm) : mutacion.nombre}</h2>
            <p class="card-subtitle">${mutacion.subtitulo}</p>
          </div>
          <button class="toggle-btn" aria-label="Expandir/contraer">+</button>
        </div>
        <div class="card-body">
          ${mutacion.secciones.map(seccion => this.createSection(seccion, shouldHighlight)).join('')}
        </div>
      </article>
    `;
  },
  ...
  createNoResults() {
    return `
      <div class="no-results visible">
        ...
        <p class="no-results-text">No se encontraron mutaciones con el criterio de búsqueda.</p>
      </div>
    `;
  },
    ...
    return `
      <footer class="footer">
        <div class="footer-content">
          <p><strong>Fuente:</strong> Contenido basado en <em>Nussbaum et al. (2023) Thompson & Thompson Genetics in Medicine, 9th ed.; Strachan & Read (2021) Human Molecular Genetics, 5th ed.</em></p>
