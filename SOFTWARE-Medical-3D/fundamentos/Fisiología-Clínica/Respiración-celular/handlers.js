...
const handlers = {
  handleClose() {
    if (window.history.length > 1) {
      ...
      window.location.href = '../../../bioquimica.html';
    }
  },
...
const render = {
    ...
    root.innerHTML = `
      ...
      <main class="main-content">
        <div class="organelos-grid" id="organelosGrid">
          ${state.filteredVias.length > 0 
            ? state.filteredVias.map((via, idx) => components.createCard(via, idx)).join('')
  ...
  renderCards() {
    ...
    if (state.filteredVias.length === 0) {
      grid.innerHTML = components.createNoResults();
    } else {
      grid.innerHTML = state.filteredVias
        .map((via, idx) => components.createCard(via, idx))
        .join('');
    }
