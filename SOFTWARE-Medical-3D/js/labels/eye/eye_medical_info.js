/**
 * ═══════════════════════════════════════════════════
 *  EYE MEDICAL INFO — Orquestador UI
 *  Plataforma Médica In Silico
 * ═══════════════════════════════════════════════════
 *
 *  Panel expandible con información médica extendida.
 *  Consume datos de window.__EYE_DATA registrados
...
function waitForReady(maxWait = 12000) {
    return new Promise((resolve, reject) => {
        const t0 = Date.now();
        (function check() {
            const engineOK = window.__EYE3D &&
                             window.__EYE3D.scene &&
                             window.__EYE3D.camera;
            const dataOK = window.__EYE_DATA &&
...
waitForReady().then(() => {
    ...
    title.textContent = '👁️ Información Médica Extendida';
    ...
    console.log(`✅ Eye Medical Info: 6 categorías, ${totalItems} elementos cargados`);

}).catch(err => {
    console.warn('⚠️ Eye Medical Info:', err);
});
