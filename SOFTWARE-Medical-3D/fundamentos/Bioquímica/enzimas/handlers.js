...
const render = {
    ...
    root.innerHTML = `
      ...
      <main class="main-content">
        <div class="organelos-grid" id="organelosGrid">
          ${state.filteredEnzimas.length > 0 
            ? state.filteredEnzimas.map((enz, idx) => components.createCard(enz, idx)).join('')
  ...
  renderCards() {
    ...
    if (state.filteredEnzimas.length === 0) {
      grid.innerHTML = components.createNoResults();
    } else {
      grid.innerHTML = state.filteredEnzimas
        .map((enz, idx) => components.createCard(enz, idx))
        .join('');
    }
