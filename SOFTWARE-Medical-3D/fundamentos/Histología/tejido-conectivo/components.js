...
const components = {
      ...
      <header class="header">
        <div class="header-content">
          ...
          <div class="header-title">
            <h1>Tipos de Tejido Conectivo</h1>
            <p class="header-subtitle">Histología · Nivel Universitario</p>
          </div>
      ...
      <section class="search-section">
        <div class="search-container">
          ...
          <input 
            ...
            class="search-input" 
            id="searchInput"
            placeholder="Buscar tejido, función, célula, enfermedad..."
  ...
  createCard(tejido, index) {
    const isExpanded = state.expandedCards.has(tejido.id);
    ...
    return `
      ...
      >
        <div class="card-header" data-id="${tejido.id}">
          <div class="card-icon">${tejido.icono}</div>
          <div class="card-title-area">
            <h2 class="card-title">${shouldHighlight ? utils.highlightText(tejido.nombre, state.searchTerm) : tejido.nombre}</h2>
            <p class="card-subtitle">${tejido.subtitulo}</p>
          </div>
          <button class="toggle-btn" aria-label="Expandir/contraer">+</button>
        </div>
        <div class="card-body">
          ${tejido.secciones.map(seccion => this.createSection(seccion, shouldHighlight)).join('')}
        </div>
      </article>
    `;
  },
    ...
    return `
      <section class="card-section clinical-section">
        ...
        <div class="clinical-content">
          <strong>🔴 Enfermedades y condiciones asociadas:</strong>
          <ul>
            ${seccion.items.map(item => `
              <li>${shouldHighlight ? utils.highlightText(item, state.searchTerm) : item}</li>
            `).join('')}
  ...
  createNoResults() {
    return `
      <div class="no-results visible">
        ...
        <p class="no-results-text">No se encontraron tejidos con el criterio de búsqueda.</p>
      </div>
    `;
  },
    ...
    return `
      <footer class="footer">
        <div class="footer-content">
          <p><strong>Fuente:</strong> Contenido basado en <em>Junqueira's Basic Histology (16th ed.); Ross & Pawlina, Histology: A Text and Atlas (8th ed.); Gartner & Hiatt, Color Atlas of Histology (7th ed.)</em></p>
