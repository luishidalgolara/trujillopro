...
const render = {
    ...
    root.innerHTML = `
      ...
      <main class="main-content">
        <div class="organelos-grid" id="organelosGrid">
          ${state.filteredRegiones.length > 0 
            ? state.filteredRegiones.map((reg, idx) => components.createCard(reg, idx)).join('')
  ...
  renderCards() {
    ...
    if (state.filteredRegiones.length === 0) {
      grid.innerHTML = components.createNoResults();
    } else {
      grid.innerHTML = state.filteredRegiones
        .map((reg, idx) => components.createCard(reg, idx))
        .join('');
    }
