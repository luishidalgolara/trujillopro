...
const components = {
      ...
      <header class="header">
        <div class="header-content">
          ...
          <div class="header-title">
            <h1>Enzimas y Catálisis</h1>
            <p class="header-subtitle">Bioquímica Enzimática · Nivel Universitario</p>
          </div>
      ...
      <section class="search-section">
        <div class="search-container">
          ...
          <input 
            ...
            class="search-input" 
            id="searchInput"
            placeholder="Buscar enzima, sustrato, cofactor, mecanismo, enfermedad..."
  ...
  createCard(enzima, index) {
    const isExpanded = state.expandedCards.has(enzima.id);
    ...
    return `
      ...
      >
        <div class="card-header" data-id="${enzima.id}">
          <div class="card-icon">${enzima.icono}</div>
          <div class="card-title-area">
            <h2 class="card-title">${shouldHighlight ? utils.highlightText(enzima.nombre, state.searchTerm) : enzima.nombre}</h2>
            <p class="card-subtitle">${enzima.subtitulo}</p>
          </div>
          <button class="toggle-btn" aria-label="Expandir/contraer">+</button>
        </div>
        <div class="card-body">
          ${enzima.secciones.map(seccion => this.createSection(seccion, shouldHighlight)).join('')}
        </div>
      </article>
    `;
  },
  ...
  createNoResults() {
    return `
      <div class="no-results visible">
        ...
        <p class="no-results-text">No se encontraron enzimas con el criterio de búsqueda.</p>
      </div>
    `;
  },
    ...
    return `
      <footer class="footer">
        <div class="footer-content">
          <p><strong>Fuente:</strong> Contenido basado en <em>Berg et al. (2023) Biochemistry, 10th ed.; Nelson & Cox (2021) Lehninger Principles of Biochemistry, 8th ed.</em></p>
