function createPipeLabelNivel2(desde, hacia, diameter, lengthMeters) {
    ...
    labelGroup.setAttribute('class', 'pipe-label-group nivel-2');
    ...
    movableGroup.setAttribute('class', 'movable-label nivel-2');
    ...
    setupLabelDragNivel2(movableGroup, guideLine, midX, midY, connectionId, line0, line1, line2);
    setupTextEditNivel2(line0, line1, line2);
    
    tracingSvg.appendChild(labelGroup);
}

function setupTextEditNivel2(line0, line1, line2) {
    line0.addEventListener('dblclick', function(e) {
        e.stopPropagation();
        const currentText = line0.textContent;
        const match = currentText.match(/i=([\d.]+)%/);
        const currentValue = match ? match[1] : '1.00';
        
        const newValue = prompt('Ingrese nueva pendiente (%):', currentValue);
        if (newValue && !isNaN(newValue)) {
            line0.textContent = `i=${parseFloat(newValue).toFixed(2)}%`;
            showStatus(`✏️ Pendiente 2° nivel actualizada: ${newValue}%`);
        }
    });
    ...
    line1.addEventListener('dblclick', function(e) {
        ...
        if (newValue && !isNaN(newValue)) {
            ...
            showStatus(`✏️ Diámetro 2° nivel actualizado: ${newValue}mm`);
        }
    });
    ...
    line2.addEventListener('dblclick', function(e) {
        ...
        if (newValue && !isNaN(newValue)) {
            ...
            showStatus(`✏️ Largo 2° nivel actualizado: ${newValue}m`);
        }
    });
}

function setupLabelDragNivel2(movableGroup, guideLine, fixedX, fixedY, connectionId, line0, line1, line2) {
    ...
    function handleMouseUp(e) {
        if (isDragging) {
            ...
            showStatus('📍 Etiqueta 2° nivel reposicionada');
...
function updateLabelForLineNivel2(line) {
    const fromId = line.getAttribute('data-from');
    const toId = line.getAttribute('data-to');
    const connectionId = `${fromId}-${toId}`;
    
    const labelGroup = document.querySelector(`.pipe-label-group.nivel-2[data-connection="${connectionId}"]`);
...
window.createPipeLabelNivel2 = createPipeLabelNivel2;
window.updateLabelForLineNivel2 = updateLabelForLineNivel2;

console.log('✅ nivel-2-labels.js cargado');