/**
 * ═══════════════════════════════════════════════════
 *  LIVER MEDICAL INFO — Orquestador UI
 *  Plataforma Médica In Silico
 * ═══════════════════════════════════════════════════
 *
 *  Panel expandible con información médica extendida.
 *  Consume datos de window.__LIVER_DATA registrados
...
function waitForReady(maxWait = 12000) {
    return new Promise((resolve, reject) => {
        const t0 = Date.now();
        (function check() {
            const engineOK = window.__LIVER3D &&
                             window.__LIVER3D.scene &&
                             window.__LIVER3D.camera;
            const dataOK = window.__LIVER_DATA &&
...
styleEl.textContent = `
.cb.med-active {
    background: rgba(184,112,74,0.18) !important;
    border-color: #b8704a !important;
    color: #b8704a !important;
}
.lmed-panel {
    position: fixed;
    top: 66px;
...
waitForReady().then(() => {
    ...
    const DATA = window.__LIVER_DATA;
    ...
    title.textContent = '🫀 Información Médica Extendida — Hígado';
    ...
    console.log(`✅ Liver Medical Info: 6 categorías, ${totalItems} elementos cargados`);

}).catch(err => {
    console.warn('⚠️ Liver Medical Info:', err);
});
