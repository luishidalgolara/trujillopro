...
const components = {
      ...
      <header class="header">
        <div class="header-content">
          ...
          <div class="header-title">
            ...
            <p class="header-subtitle">Bioquímica y Fisiología · Nivel Universitario</p>
          </div>
      ...
      <section class="search-section">
        <div class="search-container">
          ...
          <input 
            ...
            class="search-input" 
            id="searchInput"
            placeholder="Buscar ruta metabólica, enzima, sustrato, producto, enfermedad..."
  ...
  createCard(ruta, index) {
    const isExpanded = state.expandedCards.has(ruta.id);
    ...
    return `
      ...
      >
        <div class="card-header" data-id="${ruta.id}">
          <div class="card-icon">${ruta.icono}</div>
          <div class="card-title-area">
            <h2 class="card-title">${shouldHighlight ? utils.highlightText(ruta.nombre, state.searchTerm) : ruta.nombre}</h2>
            <p class="card-subtitle">${ruta.subtitulo}</p>
          </div>
          <button class="toggle-btn" aria-label="Expandir/contraer">+</button>
        </div>
        <div class="card-body">
          ${ruta.secciones.map(seccion => this.createSection(seccion, shouldHighlight)).join('')}
        </div>
      </article>
    `;
  },
  ...
  createNoResults() {
    return `
      <div class="no-results visible">
        ...
        <p class="no-results-text">No se encontraron rutas metabólicas con el criterio de búsqueda.</p>
      </div>
    `;
  },
