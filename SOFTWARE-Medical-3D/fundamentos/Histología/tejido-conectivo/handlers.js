...
const render = {
  renderAll() {
    ...
    root.innerHTML = `
      ...
      <main class="main-content">
        <div class="organelos-grid" id="tejidosGrid">
          ${state.filteredTejidos.length > 0 
            ? state.filteredTejidos.map((tejido, idx) => components.createCard(tejido, idx)).join('')
  ...
  renderCards() {
    const grid = document.getElementById('tejidosGrid');
    if (!grid) return;

    if (state.filteredTejidos.length === 0) {
      grid.innerHTML = components.createNoResults();
    } else {
      grid.innerHTML = state.filteredTejidos
        .map((tejido, idx) => components.createCard(tejido, idx))
        .join('');
    }
