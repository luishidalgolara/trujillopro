...
const render = {
    ...
    root.innerHTML = `
      ...
      <main class="main-content">
        <div class="organelos-grid" id="organelosGrid">
          ${state.filteredHormonas.length > 0 
            ? state.filteredHormonas.map((h, idx) => components.createCard(h, idx)).join('')
  ...
  renderCards() {
    ...
    if (state.filteredHormonas.length === 0) {
      grid.innerHTML = components.createNoResults();
    } else {
      grid.innerHTML = state.filteredHormonas
        .map((h, idx) => components.createCard(h, idx))
        .join('');
    }
