...
const render = {
    ...
    root.innerHTML = `
      ...
      <main class="main-content">
        <div class="organelos-grid" id="organelosGrid">
          ${state.filteredEstructuras.length > 0 
            ? state.filteredEstructuras.map((est, idx) => components.createCard(est, idx)).join('')
  ...
  renderCards() {
    ...
    if (state.filteredEstructuras.length === 0) {
      grid.innerHTML = components.createNoResults();
    } else {
      grid.innerHTML = state.filteredEstructuras
        .map((est, idx) => components.createCard(est, idx))
        .join('');
    }
