...
const render = {
    ...
    root.innerHTML = `
      ...
      <main class="main-content">
        <div class="organelos-grid" id="organelosGrid">
          ${state.filteredTemas.length > 0 
            ? state.filteredTemas.map((t, idx) => components.createCard(t, idx)).join('')
  ...
  renderCards() {
    ...
    if (state.filteredTemas.length === 0) {
      grid.innerHTML = components.createNoResults();
    } else {
      grid.innerHTML = state.filteredTemas
        .map((t, idx) => components.createCard(t, idx))
        .join('');
    }
