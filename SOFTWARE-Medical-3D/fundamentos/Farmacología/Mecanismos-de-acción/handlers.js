...
const handlers = {
  handleClose() {
    if (window.history.length > 1) {
      ...
      window.location.href = '../../../farmacologia.html';
    }
  },
...
const render = {
    ...
    root.innerHTML = `
      ...
      <main class="main-content">
        <div class="organelos-grid" id="organelosGrid">
          ${state.filteredOrganelos.length > 0 
            ? state.filteredOrganelos.map((org, idx) => components.createCard(org, idx)).join('')
  ...
  renderCards() {
    ...
    if (state.filteredOrganelos.length === 0) {
      grid.innerHTML = components.createNoResults();
    } else {
      grid.innerHTML = state.filteredOrganelos
        .map((org, idx) => components.createCard(org, idx))
        .join('');
    }
