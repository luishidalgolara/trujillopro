...
const handlers = {
  handleClose() {
    if (window.history.length > 1) {
      ...
      window.location.href = '../../../neurofisiologia.html';
    }
  },
...
const render = {
    ...
    root.innerHTML = `
      ...
      <main class="main-content">
        <div class="organelos-grid" id="organelosGrid">
          ${state.filteredConceptos.length > 0 
            ? state.filteredConceptos.map((concepto, idx) => components.createCard(concepto, idx)).join('')
  ...
  renderCards() {
    ...
    if (state.filteredConceptos.length === 0) {
      grid.innerHTML = components.createNoResults();
    } else {
      grid.innerHTML = state.filteredConceptos
        .map((concepto, idx) => components.createCard(concepto, idx))
        .join('');
    }
