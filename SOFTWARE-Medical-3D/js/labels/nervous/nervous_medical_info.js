/**
 * ═══════════════════════════════════════════════════
 *  NERVOUS SYSTEM MEDICAL INFO — Orquestador UI
...
function waitForReady(maxWait = 12000) {
    return new Promise((resolve, reject) => {
        const t0 = Date.now();
        (function check() {
            const engineOK = window.__NERVOUS3D && window.__NERVOUS3D.scene && window.__NERVOUS3D.camera;
            const dataOK = window.__NERVOUS_DATA && window.__NERVOUS_DATA.anatomia && window.__NERVOUS_DATA.fisiologia &&
                           window.__NERVOUS_DATA.patologias && window.__NERVOUS_DATA.nutricion && 
                           window.__NERVOUS_DATA.habitos && window.__NERVOUS_DATA.clinicos;
...
waitForReady().then(() => {
    const DATA = window.__NERVOUS_DATA;
    ...
    title.textContent = '⚡ Información Médica Extendida — Sistema Nervioso';
    ...
    console.log(`✅ Nervous Medical Info: 6 categorías, ${totalItems} elementos cargados`);

}).catch(err => {
    console.warn('⚠️ Nervous Medical Info:', err);
});