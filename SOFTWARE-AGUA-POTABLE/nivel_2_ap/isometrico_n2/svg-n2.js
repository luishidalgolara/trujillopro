// nivel_2_ap/isometrico_n2/svg-n2.js

IsometricWaterGeneratorN2.prototype.generateConexionNivel2 = function(x, y, label) {
    const scale = this.currentScale;
    const fontSize = Math.max(7, 10 * this.textScale);
    
    return `<g class="conexion-n2-symbol" data-type="conexion-n2">
        <circle cx="${x}" cy="${y}" r="${25*scale}" fill="#3498db" stroke="#2c3e50" stroke-width="${3*scale}"/>
        <text x="${x}" y="${y + 5*scale}" text-anchor="middle" font-family="Arial" 
            font-size="${18*scale}" font-weight="bold" fill="white">2°</text>
        <g transform="translate(${x}, ${y + 40*scale})">
            <rect x="-20" y="-10" width="40" height="20" fill="white" stroke="#2c3e50" stroke-width="1.5" rx="3"/>
...
IsometricWaterGeneratorN2.prototype.generateCalefon = function(x, y, label) {
    const scale = this.currentScale;
    const fontSize = Math.max(7, 10 * this.textScale);
...
IsometricWaterGeneratorN2.prototype.generateTermoElectrico = function(x, y, label) {
    const scale = this.currentScale;
    const fontSize = Math.max(7, 10 * this.textScale);
...
IsometricWaterGeneratorN2.prototype.generateWCSymbol = function(x, y, label) {
    const scale = this.currentScale;
    const fontSize = Math.max(7, 10 * this.textScale);
...
IsometricWaterGeneratorN2.prototype.generateLavatorio = function(x, y, label) {
    const scale = this.currentScale;
...
IsometricWaterGeneratorN2.prototype.generateDucha = function(x, y, label) {
    const scale = this.currentScale;
    const fontSize = Math.max(7, 9 * this.textScale);
...
IsometricWaterGeneratorN2.prototype.generateValvula = function(x, y, label) {
    const scale = this.currentScale;
...
IsometricWaterGeneratorN2.prototype.generateFixtureSymbol = function(x, y, type, label) {
    switch(type) {
        case 'CONEXION-N2': return this.generateConexionNivel2(x, y, label);
...
IsometricWaterGeneratorN2.prototype.generatePipeSegment = function(pipe, drawScale) {
    const scale = this.currentScale;
...
IsometricWaterGeneratorN2.prototype.generateNorthIndicator = function(x, y) {
    const scale = this.currentScale;
...
console.log('✅ Motor Isométrico NIVEL 2 - SVG cargado');