...
const render = {
    ...
    root.innerHTML = `
      ...
      <main class="main-content">
        <div class="organelos-grid" id="organelosGrid">
          ${state.filteredMutaciones.length > 0 
            ? state.filteredMutaciones.map((m, idx) => components.createCard(m, idx)).join('')
  ...
  renderCards() {
    ...
    if (state.filteredMutaciones.length === 0) {
      grid.innerHTML = components.createNoResults();
    } else {
      grid.innerHTML = state.filteredMutaciones
        .map((m, idx) => components.createCard(m, idx))
        .join('');
    }
