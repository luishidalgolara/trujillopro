...
const render = {
    ...
    root.innerHTML = `
      ...
      <main class="main-content">
        <div class="organelos-grid" id="organelosGrid">
          ${state.filteredMecanismos.length > 0 
            ? state.filteredMecanismos.map((mec, idx) => components.createCard(mec, idx)).join('')
  ...
  renderCards() {
    ...
    if (state.filteredMecanismos.length === 0) {
      grid.innerHTML = components.createNoResults();
    } else {
      grid.innerHTML = state.filteredMecanismos
        .map((mec, idx) => components.createCard(mec, idx))
        .join('');
    }
