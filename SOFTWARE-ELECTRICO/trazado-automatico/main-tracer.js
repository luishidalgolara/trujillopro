...
const AutoTracer = {
    ...
    generateIluminacion() {
        ...
        // ✅ PASO 1: Detectar infraestructura CON FILTRO DE TABLERO ILUMINACIÓN
        this.infrastructure = HierarchyDetector.detectInfrastructure(elements, 'iluminacion');
        
        if (!this.infrastructure.valid) {
            const errorMsg = this.infrastructure.errors.join('\n');
            updateStatus('❌ Infraestructura incompleta');
            alert('❌ TABLERO DE ILUMINACIÓN REQUERIDO:\n\n' + errorMsg + '\n\n' +
                  '1. Selecciona el botón "⚡ Tablero"\n' +
                  '2. Haz clic en el plano\n' +
                  '3. Selecciona "ILUMINACIÓN 💡"\n\n' +
                  'Luego podrás generar el trazado de iluminación.');
            return false;
        }
    ...
    // ========================================
    // ✨ FUNCIÓN MODIFICADA - SOLO ENCHUFES ✨
    // ✅ CON VALIDACIÓN DE TABLERO TIPO ENCHUFES
    // ========================================
    generateEnchufes() {
        ...
        // ✅ VALIDACIÓN: Verificar que exista tablero tipo "enchufes"
        const tableroEnchufes = elements.find(el => el.type === 'tablero' && el.tableroTipo === 'enchufes');
        
        if (!tableroEnchufes) {
            updateStatus('❌ Falta tablero de enchufes');
            alert('❌ TABLERO DE ENCHUFES REQUERIDO\n\n' +
                  'Para generar el trazado de enchufes necesitas colocar un TABLERO tipo ENCHUFES.\n\n' +
                  '1. Selecciona el botón "⚡ Tablero"\n' +
                  '2. Haz clic en el plano\n' +
                  '3. Selecciona "ENCHUFES 🔌" en el modal\n\n' +
                  'Luego podrás generar el trazado de enchufes.');
            return false;
        }
        
        console.log('✅ Tablero de enchufes encontrado:', tableroEnchufes);
