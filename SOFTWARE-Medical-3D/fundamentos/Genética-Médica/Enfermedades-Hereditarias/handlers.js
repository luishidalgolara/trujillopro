...
const render = {
    ...
    root.innerHTML = `
      ...
      <main class="main-content">
        <div class="organelos-grid" id="organelosGrid">
          ${state.filteredEnfermedades.length > 0 
            ? state.filteredEnfermedades.map((e, idx) => components.createCard(e, idx)).join('')
  ...
  renderCards() {
    ...
    if (state.filteredEnfermedades.length === 0) {
      grid.innerHTML = components.createNoResults();
    } else {
      grid.innerHTML = state.filteredEnfermedades
        .map((e, idx) => components.createCard(e, idx))
        .join('');
    }
