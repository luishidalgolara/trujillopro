...
function configurarDragPuntoEditable(circulo, linea) {
    ...
    function handleMouseDown(e) {
        ...
        console.log('🔴 ARRASTRANDO - Suelta o presiona ENTER para confirmar');
    }
    ...
    function handleMouseMove(e) {
        ...
        // ⭐ CAMBIO: Actualizar punto FINAL de la línea (x1, y1)
        linea.setAttribute('x1', coords.x);
        linea.setAttribute('y1', coords.y);
        ...
        if (elementoBajo) {
            ...
            if (elementoBajo.isVirtual) {
                console.log('🎯 ENTER para confirmar conexión en ángulo');
            } else {
                console.log(`🎯 ENTER para confirmar → ${elementoBajo.type || 'Cámara'}`);
            }
        } else {
            circulo.setAttribute('fill', '#f59e0b');
            console.log('⚠️ Presiona ENTER para confirmar posición');
        }
    }
    ...
    function handleKeyPress(e) {
        ...
        if (e.key === 'Enter' || e.keyCode === 13) {
            ...
            if (nuevoDestino && validarReconexion(editorPuntos.elementoOrigen, nuevoDestino)) {
                ...
                if (nuevoDestino.isVirtual) {
                    console.log('✅ Confirmado: Conexión en ángulo creada');
                } else {
                    console.log(`✅ Confirmado: Reconectado a ${nuevoDestino.type}`);
                }
            } else {
                // SOLO ACTUALIZAR LONGITUD (sin reconectar)
                actualizarSoloLongitud(linea, cx, cy);
                console.log('✅ Confirmado: Longitud actualizada');
            }
            ...
            // ✅ REGENERAR ISOMÉTRICO SI ESTÁ ABIERTO
            setTimeout(() => {
                if (window.isometricGenerator) {
                    if (window.isometricGenerator.isWindowOpen || window.isometricGenerator.isIntegrated) {
                        if (typeof regenerateIsometric === 'function') {
                            console.log('🔄 Regenerando isométrico después de edición...');
                            regenerateIsometric();
                        }
                    }
                }
        ...
        if (e.key === 'Escape' || e.keyCode === 27) {
            ...
            linea.setAttribute('x1', editorPuntos.posicionInicial.x);
            linea.setAttribute('y1', editorPuntos.posicionInicial.y);
            ...
            console.log('❌ Edición cancelada - Posición restaurada');
        }
    }
    ...
    function handleMouseUp(e) {
        ...
        console.log('⏳ Presiona ENTER para confirmar o ESC para cancelar');
    }
    ...
    circulo.addEventListener('mouseenter', () => {
        if (!editorPuntos.activo) {
            ...
            console.log('🔴 Click y arrastra para mover');
        }
    });
