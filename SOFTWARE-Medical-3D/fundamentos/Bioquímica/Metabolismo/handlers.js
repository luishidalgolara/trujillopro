...
const render = {
    ...
    root.innerHTML = `
      ...
      <main class="main-content">
        <div class="organelos-grid" id="organelosGrid">
          ${state.filteredRutas.length > 0 
            ? state.filteredRutas.map((ruta, idx) => components.createCard(ruta, idx)).join('')
  ...
  renderCards() {
    ...
    if (state.filteredRutas.length === 0) {
      grid.innerHTML = components.createNoResults();
    } else {
      grid.innerHTML = state.filteredRutas
        .map((ruta, idx) => components.createCard(ruta, idx))
        .join('');
    }
