...
const components = {
      ...
      <header class="header">
        <div class="header-content">
          ...
          <div class="header-title">
            <h1>Estructura y Función de Proteínas</h1>
            <p class="header-subtitle">Bioquímica Proteica · Nivel Universitario</p>
          </div>
      ...
      <section class="search-section">
        <div class="search-container">
          ...
          <input 
            ...
            class="search-input" 
            id="searchInput"
            placeholder="Buscar proteína, función, estructura, dominio, enfermedad..."
  ...
  createCard(proteina, index) {
    const isExpanded = state.expandedCards.has(proteina.id);
    ...
    return `
      ...
      >
        <div class="card-header" data-id="${proteina.id}">
          <div class="card-icon">${proteina.icono}</div>
          <div class="card-title-area">
            <h2 class="card-title">${shouldHighlight ? utils.highlightText(proteina.nombre, state.searchTerm) : proteina.nombre}</h2>
            <p class="card-subtitle">${proteina.subtitulo}</p>
          </div>
          <button class="toggle-btn" aria-label="Expandir/contraer">+</button>
        </div>
        <div class="card-body">
          ${proteina.secciones.map(seccion => this.createSection(seccion, shouldHighlight)).join('')}
        </div>
      </article>
    `;
  },
  ...
  createNoResults() {
    return `
      <div class="no-results visible">
        ...
        <p class="no-results-text">No se encontraron proteínas con el criterio de búsqueda.</p>
      </div>
    `;
  },
    ...
    return `
      <footer class="footer">
        <div class="footer-content">
          <p><strong>Fuente:</strong> Contenido basado en <em>Alberts et al. (2022) Molecular Biology of the Cell, 7th ed.; Berg et al. (2023) Biochemistry, 10th ed.</em></p>
