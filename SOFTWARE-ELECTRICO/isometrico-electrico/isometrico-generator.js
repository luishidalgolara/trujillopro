...
<g id="isometric-main">
    ...
    <g class="draggable-label draggable-title" transform="translate(${vb.x + vb.width/2}, ${vb.y + 40})" style="cursor: move;">
        ...
        <text x="0" y="5" text-anchor="middle" font-size="28" font-weight="bold" fill="#2c3e50">ISOMÉTRICO ELÉCTRICO</text>
    </g>
    ...
    drawPlatform(level, bounds) {
        // FUNCIÓN ELIMINADA - Ya no dibuja nada
        return '';
    },
    ...
    drawPipes() {
        ...
        this.level1Elements.forEach((el, idx) => {
            ...
            // CORREGIDO: Las luminarias están en el mismo X,Y pero el cable sube
            const elPos = {x: el.x, y: el.y, z: 0}; // Siempre en el piso primero
            const elPosTop = {x: el.x, y: el.y, z: isLuminaria ? 150 : 0}; // Altura del elemento
            
            const path = IsometricMath.generatePath(tPos, elPosTop);
            ...
            const midX = (tPos.x + elPosTop.x) / 2;
            const midY = (tPos.y + elPosTop.y) / 2;
            const midZ = (tPos.z + elPosTop.z) / 2;
        ...
        if (this.level2Elements.length > 0) {
            ...
            if (this.subidasNivel1.length > 0 && this.llegadasNivel2.length > 0) {
                ...
                this.level2Elements.forEach(el => {
                    ...
                    // CORREGIDO: Igual que nivel 1, cables suben desde el piso
                    const elPosTop = {x: el.x, y: el.y, z: isLuminaria ? 350 : 200};
                    
                    const path = IsometricMath.generatePath(llegadaPos, elPosTop);
                    const color = isLuminaria ? '#f39c12' : '#e91e63';
                    p += `<path d="${path}" stroke="${color}" stroke-width="3" fill="none" stroke-linecap="round"/>`;
                    
                    // Etiqueta N2 - DRAGGABLE
                    const midX = (llegadaPos.x + elPosTop.x) / 2;
                    const midY = (llegadaPos.y + elPosTop.y) / 2;
                    const midZ = (llegadaPos.z + elPosTop.z) / 2;
                    const midIso = IsometricMath.to2D(midX, midY, midZ);
