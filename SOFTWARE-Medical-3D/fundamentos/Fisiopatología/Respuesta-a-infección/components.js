...
const components = {
      ...
      <header class="header">
        <div class="header-content">
          ...
          <div class="header-title">
            <h1>Respuesta a Infección</h1>
            <p class="header-subtitle">Inmunología e Inflamación · Nivel Universitario</p>
          </div>
      ...
      <section class="search-section">
        <div class="search-container">
          ...
          <input 
            ...
            class="search-input" 
            id="searchInput"
            placeholder="Buscar piretógeno, citocina, célula, receptor, enfermedad..."
  ...
  createCard(concepto, index) {
    const isExpanded = state.expandedCards.has(concepto.id);
    ...
    return `
      ...
      >
        <div class="card-header" data-id="${concepto.id}">
          <div class="card-icon">${concepto.icono}</div>
          <div class="card-title-area">
            <h2 class="card-title">${shouldHighlight ? utils.highlightText(concepto.nombre, state.searchTerm) : concepto.nombre}</h2>
            <p class="card-subtitle">${concepto.subtitulo}</p>
          </div>
          <button class="toggle-btn" aria-label="Expandir/contraer">+</button>
        </div>
        <div class="card-body">
          ${concepto.secciones.map(seccion => this.createSection(seccion, shouldHighlight)).join('')}
        </div>
      </article>
    `;
  },
  ...
  createNoResults() {
    return `
      <div class="no-results visible">
        ...
        <p class="no-results-text">No se encontraron conceptos con el criterio de búsqueda.</p>
      </div>
    `;
  },
    ...
    return `
      <footer class="footer">
        <div class="footer-content">
          <p><strong>Fuente:</strong> Contenido basado en <em>Abbas, Lichtman & Pillai Cellular and Molecular Immunology, 10th ed. (2022); Murphy & Weaver Janeway's Immunobiology, 10th ed. (2022); Mandell, Douglas & Bennett's Infectious Diseases, 9th ed. (2020)</em></p>
