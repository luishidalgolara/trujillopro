...
const components = {
      ...
      <header class="header">
        <div class="header-content">
          ...
          <div class="header-title">
            <h1>Anatomía Topográfica</h1>
            <p class="header-subtitle">Relaciones Espaciales · Nivel Universitario</p>
          </div>
      ...
      <section class="search-section">
        <div class="search-container">
          ...
          <input 
            ...
            class="search-input" 
            id="searchInput"
            placeholder="Buscar región, límites, vasos, nervios, abordaje quirúrgico..."
  ...
  createCard(region, index) {
    const isExpanded = state.expandedCards.has(region.id);
    ...
    return `
      ...
      >
        <div class="card-header" data-id="${region.id}">
          <div class="card-icon">${region.icono}</div>
          <div class="card-title-area">
            <h2 class="card-title">${shouldHighlight ? utils.highlightText(region.nombre, state.searchTerm) : region.nombre}</h2>
            <p class="card-subtitle">${region.subtitulo}</p>
          </div>
          <button class="toggle-btn" aria-label="Expandir/contraer">+</button>
        </div>
        <div class="card-body">
          ${region.secciones.map(seccion => this.createSection(seccion, shouldHighlight)).join('')}
        </div>
      </article>
    `;
  },
    ...
    return `
      <section class="card-section clinical-section">
        ...
        <div class="clinical-content">
          <strong>${seccion.subtitulo || '⚕️ Importancia Quirúrgica:'}</strong>
          <ul>
            ${seccion.items.map(item => `
              <li>${shouldHighlight ? utils.highlightText(item, state.searchTerm) : item}</li>
            `).join('')}
  ...
  createNoResults() {
    return `
      <div class="no-results visible">
        ...
        <p class="no-results-text">No se encontraron regiones anatómicas con el criterio de búsqueda.</p>
      </div>
    `;
  },
    ...
    return `
      <footer class="footer">
        <div class="footer-content">
          <p><strong>Fuente:</strong> Contenido basado en <em>Gray's Anatomy (2021), Moore's Clinically Oriented Anatomy (2023), Netter's Atlas of Human Anatomy (2023)</em></p>
