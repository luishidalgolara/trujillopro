...
const components = {
      ...
      <header class="header">
        <div class="header-content">
          ...
          <div class="header-title">
            <h1>Regulación de Presión Arterial</h1>
            <p class="header-subtitle">Fisiología Clínica · Nivel Universitario</p>
          </div>
      ...
      <section class="search-section">
        <div class="search-container">
          ...
          <input 
            ...
            class="search-input" 
            id="searchInput"
            placeholder="Buscar mecanismo, hormona, receptor, enfermedad..."
  ...
  createCard(mecanismo, index) {
    const isExpanded = state.expandedCards.has(mecanismo.id);
    const shouldHighlight = state.searchTerm.length > 0;

    return `
      <article 
        class="organelo-card ${isExpanded ? 'expanded' : ''}" 
        data-id="${mecanismo.id}"
        style="animation-delay: ${index * CONFIG.animationDelay}ms"
      >
        <div class="card-header" data-id="${mecanismo.id}">
          <div class="card-icon">${mecanismo.icono}</div>
          <div class="card-title-area">
            <h2 class="card-title">${shouldHighlight ? utils.highlightText(mecanismo.nombre, state.searchTerm) : mecanismo.nombre}</h2>
            <p class="card-subtitle">${mecanismo.subtitulo}</p>
          </div>
          <button class="toggle-btn" aria-label="Expandir/contraer">+</button>
        </div>
        <div class="card-body">
          ${mecanismo.secciones.map(seccion => this.createSection(seccion, shouldHighlight)).join('')}
        </div>
      </article>
    `;
  },
  ...
  createNoResults() {
    return `
      <div class="no-results visible">
        ...
        <p class="no-results-text">No se encontraron mecanismos con el criterio de búsqueda.</p>
      </div>
    `;
  },
    ...
    return `
      <footer class="footer">
        <div class="footer-content">
          <p><strong>Fuente:</strong> Contenido basado en <em>Guyton & Hall Textbook of Medical Physiology, 14th ed. (2021); Boron & Boulpaep Medical Physiology, 3rd ed. (2017); Kaplan's Clinical Hypertension, 11th ed. (2022)</em></p>
